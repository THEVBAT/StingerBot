...
module.exports = class Note extends commands {
    ...
    static UserNoter (message, nUser) {
        let notes = JSON.parse(fs.readFileSync("../Stingerbot/JSON/note.json", "utf8"))
        let names = JSON.parse(fs.readFileSync("../Stingerbot/JSON/UserNote.json", "utf8"))
        ...
        fs.writeFile("../Stingerbot/JSON/note.json", JSON.stringify(notes), (err) => {
            if (err) {
                console.log(err);
            }
        });
